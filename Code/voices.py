...
# Buttons to control the video feed
...
with col5:
    if st.button("Save Character"):
        # Save the current recognized character to the result array
...
if st.session_state["video_active"]:
    ...
    while st.session_state["video_active"]:
        ...
        recognized_text.text(f"Recognized Character: {st.session_state['current_label']}")
        final_result.text(f"Current Text: {''.join(st.session_state['arr'])}")

        # Break the loop if the video feed is stopped
